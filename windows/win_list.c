...
/*link this libraries explicitly on the command line if you're using MinGW
gcc win_list.c -o win_list.exec -liphlpapi -lws2_32.*/
...
int main()
{
	...
	do{
		...
		if (!adapters) {
			printf("Could't allocate %ld bytes for adapters.\n", asize);
			WSACleanup();
			return -1
		}
